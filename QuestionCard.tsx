...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, userAnswer, onAnswer }) => {
  ...
  const getOptionStyle = (key: 'A' | 'B' | 'C' | 'D') => {
    if (!isAnswered) {
      return "bg-white active:bg-indigo-50 border-slate-200 text-slate-700";
    }
      ...
      <div className="p-4 md:p-8 space-y-4 md:space-y-6">
        ...
        <div className="grid grid-cols-1 gap-2 md:gap-3">
          {(Object.keys(question.options) as Array<'A' | 'B' | 'C' | 'D'>).map((key) => (
            ...
            >
              <div className={`w-7 h-7 md:w-9 md:h-9 flex-shrink-0 rounded-lg flex items-center justify-center font-bold mr-3 border transition-colors text-sm md:text-base ${
                !isAnswered 
        ...
        {/* Feedback Section - More compact on mobile */}
